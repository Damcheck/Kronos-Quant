...
# ---------------------------------------------------------------------------
# MT5 Credential Namespace (brief §3.7 / §7)
#
# Reuses the existing Fernet-encrypted auth.json store.  MT5 credentials
# are stored under the "mt5" key:
#   {
#     "mt5": {
#       "login":    "12345678",
...
def get_mt5_credentials() -> dict | None:
    """Retrieve MT5 login/password/server from auth.json or env vars.

    Returns a dict with 'login', 'password' (decrypted), 'server', or None
    if MT5 credentials are not configured.
    """
    import json
    import os

    # Priority 1: Environment Variables (Required for Cloud Deployments)
    env_login = os.environ.get("FORVEN_MT5_LOGIN")
    env_password = os.environ.get("FORVEN_MT5_PASSWORD")
    env_server = os.environ.get("FORVEN_MT5_SERVER")
    ...
    # Priority 2: Local auth.json file
    auth_file = Path(cfg.AUTH_FILE)
    if not auth_file.exists():
        return None
    ...
    try:
        ...
        log.warning("Could not parse auth.json for MT5 credentials")
        return None

    mt5_creds = auth.get("mt5")
    if not isinstance(mt5_creds, dict):
        return None
    ...
    return {
        "login": str(login),
        "password": decrypt_secret(str(password)),
        "server": str(server),
    }


def set_mt5_credentials(login: str, password: str, server: str) -> None:
    """Store MT5 credentials in auth.json with the password encrypted.

    Uses the same Fernet scheme as all other Forven credentials.
    """
    import json
    ...
    auth["mt5"] = {
        "login": str(login),
        "password": encrypt_secret(str(password)),
        "server": str(server),
    }
    ...
    log.info("MT5 credentials stored in %s", auth_file)
